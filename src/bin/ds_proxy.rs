...
fn main() {
    ...
    if args.cmd_proxy {
        ...
        encrypt::proxy::main(listen_adress, listen_port, upstream_base_url, noop).unwrap();
    } else if args.cmd_encrypt {
        encrypt::file::encrypt(args.arg_input_file.unwrap(), args.arg_output_file.unwrap(), noop);
    } else if args.cmd_decrypt {
        encrypt::file::decrypt(args.arg_input_file.unwrap(), args.arg_output_file.unwrap(), noop);
    }
}
