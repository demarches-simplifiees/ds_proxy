...
pub fn encrypt(config: &Config) {
    ...
    // our source file
    let read = fs.read(config.input_file.clone().unwrap(), Default::default());

    let encoder = Encoder::new(config.key.clone(), config.chunk_size, Box::new(read));

    // default writes options to create a new file
    let write = fs.write(config.output_file.clone().unwrap().clone(), Default::default());
...
pub fn decrypt(config: &Config) {
    ...
    // default writes options to create a new file
    let write = fs.write(config.output_file.clone().unwrap(), Default::default());
